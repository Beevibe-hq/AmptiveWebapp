...
async function run() {
  ...
  // test 2: support_enabled
  const r2 = await fetch('https://amptive-staging.getamptive.com/api/v1/users/me', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ support_enabled: true })
  }).then(r => r.json());
  console.log("PATCH support_enabled:", r2);
