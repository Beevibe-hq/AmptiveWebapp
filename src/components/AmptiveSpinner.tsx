...
export function AmptiveSpinner({ className = 'h-8 w-8', size }: AmptiveSpinnerProps) {
  // If consumer passed animate-spin, replace it with animate-pulse because the logo doesn't spin well
  let finalClass = className.replace(/\banimate-spin\b/g, 'animate-pulse');
  
  // If no animation is passed, add pulse by default
  ...
  return (
    <svg
      ...
      aria-label="Loading..."
      style={{
          // If a custom size is passed via prop (e.g., from Lucide react pattern), apply it
